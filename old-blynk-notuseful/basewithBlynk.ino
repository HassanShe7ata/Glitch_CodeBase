...
// Your authentication token (get this from Blynk Legacy server after creating
// project)
...
// =================== CAMERA ESP-NOW ===================
// ⚠️  MUST UPDATE: Run camera once and read Serial Monitor for "CAMERA MAC:
// xx:xx:xx:xx:xx:xx" Replace the FF placeholder below with the actual 6-byte
// MAC address. ESP-NOW cannot use broadcast (FF:FF:FF:FF:FF:FF) as a peer
// address.
...
void OnDataRecv(const esp_now_recv_info_t *recvInfo, const uint8_t *data,
                int len) {
...
// ===================== STEPPED SERVO CONTROL (BUTTONS) =====================
// Each servo gets an UP button and a DOWN button (push mode).
// One press = one step. Hold = repeats.
// Step size per servo:
//   Servo 0 (base rotation):  5°
//   Servo 1 (shoulder):       3°
//   Servo 2 (elbow):           3°
//   Servo 3 (wrist):           2°
...
static void servoStep(int idx, int dir) {
  ...
  Serial.printf("[SERVO] %d → %.0f°\n", idx, newAng);
}

// V28 — Servo 0 UP / V29 — Servo 0 DOWN
BLYNK_WRITE(V28) { if (param.asInt()) servoStep(0,  1); }
BLYNK_WRITE(V29) { if (param.asInt()) servoStep(0, -1); }

// V30 — Servo 1 UP / V31 — Servo 1 DOWN
BLYNK_WRITE(V30) { if (param.asInt()) servoStep(1,  1); }
BLYNK_WRITE(V31) { if (param.asInt()) servoStep(1, -1); }

// V32 — Servo 2 UP / V33 — Servo 2 DOWN
BLYNK_WRITE(V32) { if (param.asInt()) servoStep(2,  1); }
BLYNK_WRITE(V33) { if (param.asInt()) servoStep(2, -1); }

// V34 — Servo 3 UP / V35 — Servo 3 DOWN
BLYNK_WRITE(V34) { if (param.asInt()) servoStep(3,  1); }
BLYNK_WRITE(V35) { if (param.asInt()) servoStep(3, -1); }
  ...
  if (autonomousMode && autoTrigger == 1) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      if (lastPoseReply.color == targets[i].colorCode) {
        ...
        // Wait for arm to complete (cameraGuidedPickup on arm handles its own
        // goHome)
        unsigned long t0 = millis();
