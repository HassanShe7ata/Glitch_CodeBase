...
// =================== CAMERA ESP-NOW ===================
// ΓÜá∩╕Å  MUST UPDATE: Run camera once and read Serial Monitor for "CAMERA MAC: xx:xx:xx:xx:xx:xx"
...
// ESP-NOW packet: Base ΓåÆ Camera (scan request)
...
// ESP-NOW packet: Camera ΓåÆ Base (pose reply)
...
// ESP-NOW packet: Base ΓåÆ Arm (camera pose data for guided IK)
...
// ESP-NOW receive callback ΓÇö handles pose replies from camera
...
static bool alignToQR() {
    ...
    // Step 2: Align yaw ΓÇö strafe left/right until centered on QR
    ...
    // Step 3: Approach ΓÇö move forward until close enough
...
// V28 ΓÇö Servo Index selector (slider 0-4, stores value for V30)
int selectedServo = 0;
BLYNK_WRITE(V28) {
...
// V29 ΓÇö Servo Angle selector (slider 0-180, stores value for V30)
...
// V30 ΓÇö Send Servo Move command (button)
BLYNK_WRITE(V30) {
    if (param.asInt()) {
...
// V21 ΓÇö Trigger QR scan (ask camera to detect QR codes)
BLYNK_WRITE(V21) {
    if (param.asInt()) {
        sendScanRequest(0); // mode=0: QR scan
...
// V22 ΓÇö Trigger platform scan (ask camera to detect square platform)
BLYNK_WRITE(V22) {
    if (param.asInt()) {
...
// V26 ΓÇö Move arm to scan position (camera looks forward-down)
BLYNK_WRITE(V26) {
    if (param.asInt()) {
        sendCommandToArm("S");
    ...
    if (autonomousMode && autoTrigger == 1)
    {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (lastPoseReply.color == targets[i].colorCode) {
                Serial.printf("[AUTO] Color match: %s ΓÇö sending camera pose to arm\n", targets[i].label);
                ...
                int ci = lastPoseReply.color;
                if (ci > 3) ci = 0;
                Serial.printf("[AUTO] Color mismatch: expected %s, camera sees %s ΓÇö skipping\n",
